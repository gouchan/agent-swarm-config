...
export async function researchCommand(ctx: Context): Promise<void> {
  ...
  const thinking = await ctx.reply(`🔍 Searching X for "${query}"...`);
  ...
  try {
    ...
    if (result.tweets.length === 0) {
      await ctx.api.editMessageText(
        ctx.chat!.id,
        thinking.message_id,
        `No tweets found for "${query}". Try a broader search term.`
      );
      return;
    }
    ...
    await ctx.api.editMessageText(ctx.chat!.id, thinking.message_id, header + "\n" + body, {
      parse_mode: "HTML",
